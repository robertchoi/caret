...
echo.
echo Starting vLLM server with meta-llama/Llama-4-Maverick-17B-128E-Instruct-FP8...
...
docker run -d --name vllm-llama4-maverick-fp8 --gpus all ^
    --ipc=host ^
    --shm-size 16g ^
...
echo.
echo vLLM server started in detached mode with name 'vllm-llama4-maverick-fp8'.
echo Use 'docker logs vllm-llama4-maverick-fp8 -f' to view logs.
echo Use 'docker stop vllm-llama4-maverick-fp8' to stop the server.
pause
