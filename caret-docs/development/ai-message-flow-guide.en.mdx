# AI Message Flow Guide

**Date**: 2025-01-28
**Author**: Alpha
**Audience**: Caret Developers

## 📋 **Overview**

This document details the entire process of how users send messages to the AI and receive responses in Caret. It covers the complete flow from Frontend → Backend → AI API → Backend → Frontend.
...
- Improve ability to pinpoint issues during debugging
...
sequenceDiagram
    ...
    Note over User, ChatRow: 📤 Step 1: User Message Transmission
    ...
    Note over User, ChatRow: 📡 Step 4: State Transmission
...
stateDiagram-v2
    ...
    ToolDetected --> AskWaiting: ask() Call
    AskWaiting --> UIUpdated: WebView State Update
    UIUpdated --> UserInput: Waiting for User Response
    
    UserInput --> ResponseReceived: User Feedback
    ResponseReceived --> Processing: Start Next Processing
...
## 📤 **Step 1: User Message Transmission (Frontend → Backend)**
...
```typescript
// When user inputs message and clicks send button
...
```typescript
// Extension receives WebView messages and forwards to Task
case "newTask":
...
async generateFromJsonSections(
    ...
    // 2. Generate dynamic sections (MCP, system information)
...
async *attemptApiRequest(previousApiReqIndex: number): ApiStream {
    // 1. Construct system prompt + conversation history
...
## 📡 **Step 4: Backend → Frontend State Transmission**
    ...
    if (lastMessage?.type === "ask") {
        switch (lastMessage.ask) {
            case "chatbot_mode_respond":
                ...
                setClineAsk("chatbot_mode_respond")   // Set Ask type
                setEnableButtons(false)               // Button enabled state
                break
        }
    }
}, [modifiedMessages])
    ...
    switch (message.ask) {
        case "chatbot_mode_respond": {
            ...
            try {
                ...
                response = message.text // Handle legacy message
            }
...
### **6.1 User Input Transmission**

When the user inputs feedback for the AI response, it is sent back to the Backend through the same process as **Step 1**:
...
1.  **If AI does not respond**:
    ...
    - Is the tool processing case implemented in the Backend? (`Task.ts`)
