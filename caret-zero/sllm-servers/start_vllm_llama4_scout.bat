...
echo.
echo Starting vLLM server with meta-llama/Llama-4-Scout-17B-16E-Instruct...
...
docker run -d --name vllm-llama4-scout --gpus all ^
    --shm-size 16g ^
...
echo.
echo vLLM server started in detached mode with name 'vllm-llama4-scout'.
echo Use 'docker logs vllm-llama4-scout -f' to view logs.
echo Use 'docker stop vllm-llama4-scout' to stop the server.
pause
