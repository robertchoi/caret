...
export const PersonaAvatar: React.FC<PersonaAvatarProps> = ({
	...
	// Request persona data when component mounts (only if not in test mode)
	useEffect(() => {
		if (testPersona === null) {
			caretWebviewLogger.debug("PersonaAvatar: Requesting template characters")
	...
	useEffect(() => {
		...
		const handleMessage = (event: MessageEvent) => {
			...
			if (message.type === "RESPONSE_TEMPLATE_CHARACTERS") {
				...
				caretWebviewLogger.debug("PersonaAvatar: Received template characters:", characters)

				// CARET MODIFICATION: 새로운 단순한 시스템에서는 첫 번째 character만 사용 (current persona)
				// 안전한 null/undefined 체크
				if (Array.isArray(characters) && characters.length > 0) {
					const currentPersona = characters[0] // 백엔드에서 "current" 페르소나 하나만 반환
					caretWebviewLogger.debug("PersonaAvatar: Using current persona:", currentPersona.character)
					setSelectedPersona(currentPersona)
					setIsLoading(false)
				} else {
					caretWebviewLogger.warn("PersonaAvatar: Invalid or empty characters payload:", characters)
				}
			}
