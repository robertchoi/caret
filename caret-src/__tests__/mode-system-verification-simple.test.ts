...
describe("✅ GREEN - Mode System Verification (Simplified)", () => {
	...
	it("✅ extensionPath가 있으면 Caret JSON 시스템 사용 시도", async () => {
		...
		// 실제 로그 패턴으로 수정
		const caretLogFound = consoleSpy.mock.calls.some((call) => call[0] && call[0].includes("CaretSystemPrompt"))

		const fallbackLogFound = consoleErrorSpy.mock.calls.some(
			(call) => call[0] && call[0].includes("JSON generation failed, falling back to Cline original"),
		)

		// Caret 시스템 시도는 했어야 함 (성공 여부와 관계없이)
		// 실제로는 extensionPath만 있으면 시도하므로 이 테스트는 통과해야 함
