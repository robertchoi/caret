...
-   **[메시지 처리 아키텍처 가이드](./message-processing-architecture.mdx)**: Cline의 gRPC 기반 메시지 처리 구조 완전 분석
    -   프론트엔드-백엔드 통신 패턴
    -   gRPC vs 레거시 메시지 비교
    -   API Configuration 저장 플로우
    -   스트리밍 및 이벤트 기반 패턴
    -   개발 가이드라인 및 마이그레이션 방법
...
-   [링크 관리 시스템 가이드](./link-management-guide.mdx)
-   [다국어 지원 가이드](./locale.mdx)
...
-   **[🏛️ Caret 아키텍처 가이드](./caret-architecture-and-implementation-guide.mdx)**: Fork 기반 아키텍처 개요
-   **[🔧 확장 아키텍처](./extension-architecture.mmd)**: VSCode 확장 구조 다이어그램
-   **[🧩 컴포넌트 아키텍처 원칙](./component-architecture-principles.mdx)**: UI 컴포넌트 개발 원칙 및 가이드라인
