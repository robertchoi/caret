...
- regex: (required) The regular expression pattern to search for. Uses standard regex syntax.
- file_pattern: (optional) Glob pattern to filter files (e.g., "*.ts" for TypeScript files). If not provided, it will search all files.
Usage:
<search_files>
<path>Directory path here</path>
<regex>Your regex pattern here</regex>
<file_pattern>*.extension (optional)</file_pattern>
</search_files>

## list_files
Description: Request to list the contents of a directory. This tool shows all files and subdirectories in the specified path, which can help you understand the project structure and locate files you need to work with.
Parameters:
- path: (required) The path of the directory to list (relative to the current working directory ${cwd.toPosix()})
- recursive: (optional) Whether to list files recursively in subdirectories. Use "true" for recursive listing, or omit for top-level only.
Usage:
<list_files>
<path>Directory path here</path>
...
Description: Request to list the names of all code definitions (classes, functions, methods, etc.) in source code files within a directory. This tool provides a quick overview of the codebase structure and helps you understand what components are available in the project.
Parameters:
- path: (required) The path of the directory to analyze (relative to the current working directory ${cwd.toPosix()})
Usage:
<list_code_definition_names>
<path>Directory path here</path>
</list_code_definition_names>
...
- You cannot \`cd\` into a different directory to complete a task. You are stuck operating from '${cwd.toPosix()}'.
